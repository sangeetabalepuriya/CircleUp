...
export default function CommentDialog({ open, setOpen }) {
  ...
  async function sendMessageHandler() {
    try {
      const res = await axios.post(`https://circleup-3wqg.onrender.com/post/comment/${selectedPost?._id}`, { text }, {
        headers: {
  ...
  async function handleFollowToggle() {
    try {
      const res = await axios.post(
        `https://circleup-3wqg.onrender.com/user/followOrUnFollow/${selectedPost.author._id}`,
        {},
        { withCredentials: true }
      );
  ...
  async function deletePostHandler() {
    try {
      const res = await axios.delete(`https://circleup-3wqg.onrender.com/post/delete/${selectedPost?._id}`, { withCredentials: true });
      if (res.data.success) {
